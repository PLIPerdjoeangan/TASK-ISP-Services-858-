...
class USSDApp(tk.Tk):
    ...
    def transfer_pulsa_screen(self):
        ...
        submit_button = tk.Button(button_frame, text="Submit", command=self.ask_amount_screen, width=10)
        submit_button.pack(side=tk.LEFT, padx=5)

        back_button = tk.Button(button_frame, text="Back", command=self.menu_screen, width=10)
        back_button.pack(side=tk.LEFT, padx=5)

    def ask_amount_screen(self):
        self.destination_number = self.phone_entry.get()
        if not self.destination_number:
            return

        self.clear_window()

        amount_label = tk.Label(self, text=f"Masukkan jumlah Transfer Pulsa \n ke {self.destination_number}:", font=("Arial", 12))
        amount_label.pack(pady=10)

        self.amount_entry = tk.Entry(self, font=("Arial", 14), width=15)
        self.amount_entry.pack(pady=10)

        button_frame = tk.Frame(self)
        button_frame.pack(pady=10)

        submit_button = tk.Button(button_frame, text="Submit", command=self.confirm_transfer_screen, width=10)
        submit_button.pack(side=tk.LEFT, padx=5)

        back_button = tk.Button(button_frame, text="Back", command=self.transfer_pulsa_screen, width=10)
        back_button.pack(side=tk.LEFT, padx=5)

    def confirm_transfer_screen(self):
        self.amount = self.amount_entry.get()
        if not self.amount:
            return

        
        formatted_amount = "{:,}".format(int(self.amount))

        self.clear_window()

        confirmation_label = tk.Label(self, text=f"Transfer Rp {formatted_amount} ke {self.destination_number}?\nKonfirmasi:", font=("Arial", 12))
        confirmation_label.pack(pady=10)

        button_frame = tk.Frame(self)
        button_frame.pack(pady=10)

        confirm_button = tk.Button(button_frame, text="Yes", command=self.complete_transfer, width=10)
        confirm_button.pack(side=tk.LEFT, padx=5)

        cancel_button = tk.Button(button_frame, text="No", command=self.phone_screen, width=10)
    ...
    def complete_transfer(self):
        ...
        confirmation_label = tk.Label(self, text=f"Pulsa sebanyak Rp {formatted_amount}\nberhasil di kirim ke nomor {self.destination_number}!", font=("Arial", 12), fg="green", anchor='w', justify='left')
